...
#define DataPort P0 //数码管数据端口 P0
sbit LATCH1=P2^0; // 74HC573段锁
sbit DS =P3 ^ 3; // DS18B20

unsigned char code dofly_DuanMa[10]={0x3f,0x06,0x5b,0x4f,0x66,0x6d,0x7d,0x07,0x7f,0x6f};// 显示段码值0~9
unsigned char code dofly_WeiMa[]={0xef, 0xdf, 0xbf, 0x7f,   0xfe,0xfd,0xfb,0xf7};//位码
...
void Dispaly(int p1, int p2, int p3, int p4, int point);//数码管 显示 数字
void Delay_us(int t); // 延时 1us
void Delay_ms(int t); // 延时 1ms


void main()
{
	int test = 0, num = 0; // 需要显示的值

	while(1)//不断循环显示
	{
		num = Ds_Result(); // 需要显示的值
...
unsigned int Ds_Result()
{
	float tp = 0;
	unsigned int temp = 0;
	unsigned char LSB = 0,MSB = 0;//用来储存数据的第八位与高八位
